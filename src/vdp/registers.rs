...
impl VdpRegisters {
    ...
    /// Retorna o valor do registrador (índice 0x00..0x1F)
    pub fn get(&self, index: usize) -> u8 {
        self.regs[index]
    }

    /// Compatibilidade usada nos testes: define um valor de registrador
    ...
    /// Compatibilidade usada nos testes: define o comprimento do DMA
    pub fn set_dma_length(&mut self, length: u16) {
    ...
    fn process_register_change(&mut self, index: usize, old_value: u8, new_value: u8) {
        ...
        match index {
            ...
            23 => {
                ...
                self.dma_source =
                    (self.dma_source & 0x00FFFF) | (((new_value & 0x7F) as u32) << 16);
    ...
    pub fn read_control_port(&mut self) -> u8 {
        ...
        self.status
            .remove(VdpStatus::VBLANK | VdpStatus::HBLANK | VdpStatus::LINE_IRQ);
    ...
    fn increment_address(&mut self) {
        match self.current_address.access_type {
            VdpAccessType::VRAM => {
                self.current_address.addr = self
                    .current_address
                    .addr
    ...
    fn update_fifo_status(&mut self) {
        if self.fifo.is_empty() {
            ...
            self.status
                .remove(VdpStatus::FIFO_EMPTY | VdpStatus::FIFO_FULL);
        }
    }
    ...
    pub fn debug_info(&self) -> Vec<String> {
        ...
        info.push(format!(
            "Display: {}",
            if self.display_enabled() { "ON" } else { "OFF" }
        ));
        info.push(format!(
            "Mode: {}",
            if self.mode_40_cell { "H40" } else { "H32" }
        ));
